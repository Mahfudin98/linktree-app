...
<div
  class="min-h-screen bg-[#050a0e] flex items-start justify-center pt-8 px-4 pb-16 font-['Share_Tech_Mono',_monospace] relative overflow-hidden"
>
  <!-- Scanlines overlay -->
  <div
  ...
  <!-- Grid background -->
  <div
    class="fixed inset-0 pointer-events-none z-0 bg-[linear-gradient(rgba(0,255,255,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,255,0.04)_1px,transparent_1px)] bg-[size:40px_40px]"
    aria-hidden="true"
  ></div>
  ...
  <div class="w-full max-w-[520px] flex flex-col items-center relative z-10">
    <!-- System status bar -->
    <div
      class="flex items-center justify-between gap-2 bg-cyan-400/5 border border-cyan-400/20 rounded px-3.5 py-1.5 mb-8 text-[0.65rem] text-cyan-400 tracking-widest w-full"
    >
      <span
        class="w-1.5 h-1.5 rounded-full bg-[#00ff41] shadow-[0_0_6px_#00ff41] animate-[pulse_2s_ease_infinite] shrink-0"
      ></span>
      <span class="flex-1 text-center">SYS_ONLINE // LINK_TREE v2.0</span>
      <span class="text-cyan-400/50"
        >{new Date().toLocaleTimeString("en-US", { hour12: false })}</span
      >
    </div>
    ...
    <div class="relative w-24 h-24 mb-6">
      {#if data.avatar}
        <img
          src={data.avatar}
          alt={data.name}
          class="w-24 h-24 rounded object-cover saturate-0 brightness-110 contrast-110 mix-blend-screen relative z-10"
        />
      {:else}
        <div
          class="w-24 h-24 rounded bg-[#0a1628] text-cyan-400 flex items-center justify-center text-[2.5rem] font-['Orbitron'] font-black drop-shadow-[0_0_20px_#00ffff] relative z-10"
        >
          {data.name.charAt(0).toUpperCase()}
        </div>
      {/if}
      <div
        class="absolute -inset-[3px] border-2 border-cyan-400 rounded z-0 shadow-[0_0_15px_rgba(0,255,255,0.4),inset_0_0_15px_rgba(0,255,255,0.05)]"
      ></div>
      <div
        class="absolute -inset-[3px] border-2 border-[#ff0080] rounded z-0 opacity-40 animate-[glitchShift_4s_steps(1)_infinite]"
        aria-hidden="true"
      ></div>
    </div>
    ...
    <div class="flex items-baseline gap-2 mb-3">
      ...
      <h1
        class="font-['Orbitron'] text-2xl font-black text-cyan-400 tracking-wider m-0 uppercase drop-shadow-[0_0_20px_rgba(0,255,255,0.6)]"
      >
        {data.name}
      </h1>
    </div>
    ...
    {#if data.bio}
      <p
        class="text-[0.8rem] text-[#00ff41]/75 text-center leading-[1.6] max-w-[380px] m-0 mb-6"
      >
        <span class="text-[#00ff41]">&gt; </span>{data.bio}
        <span
          class="inline-block animate-[blink_1s_step-start_infinite] text-cyan-400"
          >_</span
        >
      </p>
    {/if}
    ...
    <div class="w-full flex flex-col gap-2">
      {#each data.links as link, i}
        ...
        >
          <div
            class="absolute inset-0 bg-gradient-to-r from-transparent via-cyan-400/5 to-transparent -translate-x-[100%] transition-transform duration-400 group-hover:translate-x-[100%]"
          ></div>
          <span class="text-[#ff0080] text-[0.7rem] shrink-0 min-w-[30px]"
          ...
          <span
            class="text-[0.65rem] text-[#ff0080] opacity-0 -translate-x-2 transition-all duration-150 shrink-0 group-hover:opacity-100 group-hover:translate-x-0"
            >EXECUTE &gt;&gt;</span
          >
        </a>
      {/each}
